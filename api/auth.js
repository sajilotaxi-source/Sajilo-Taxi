...
function base32Encode(buffer) { let bits=0,bitLength=0,base32='';for(let i=0;i<buffer.length;i++){bits=(bits<<8)|buffer[i];bitLength+=8;while(bitLength>=5){base32+=base32Chars[(bits>>>(bitLength-5))&31];bitLength-=5;}}if(bitLength>0){base32+=base32Chars[(bits<<(5-bitLength))&31];}return base32;}
function base32Decode(base32){const charMap={};for(let i=0;i<base32Chars.length;i++){charMap[base32Chars[i]]=i;}let bits=0,bitLength=0;const data=[];base32=base32.toUpperCase().replace(/ /g,'');for(let i=0;i<base32.length;i++){const charVal=charMap[base32[i]];if(charVal===undefined)continue;bits=(bits<<5)|charVal;bitLength+=5;if(bitLength>=8){data.push((bits>>>(bitLength-8))&255);bitLength-=8;}}return Buffer.from(data);}
function verifyOtp(secret,token,window=1){if(!secret||!token)return false;try{const decodedSecret=base32Decode(secret);const period=30;const counter=Math.floor(Date.now()/1000/period);for(let i=-window;i<=window;i++){const timeBuffer=Buffer.alloc(8);timeBuffer.writeBigUInt64BE(BigInt(counter+i),0);const hmac=crypto.createHmac('sha1',decodedSecret);hmac.update(timeBuffer);const digest=hmac.digest();const offset=digest[digest.length-1]&0xf;const code=(digest.readUInt32BE(offset)&0x7fffffff)%1000000;if(code.toString().padStart(6,'0')===token){return true;}}}catch(e){console.error("OTP verification error:",e);return false;}return false;}
...
const initialStateCopy = JSON.parse(JSON.stringify(initialData));

// --- Server-Side State Reducer ---
function appReducer(currentState, action) {
    const getCoords = (locName, cs) => cs.customLocationCoordinates[locName] || locationCoordinates[locName];
    switch (action.type) {
        case 'ADD_CAB': { const cab = action.payload; const loc = getCoords(cab.from, currentState); const dest = getCoords(cab.to, currentState); return { ...currentState, cabs: [...currentState.cabs, { ...cab, id: Date.now(), location: loc, destination: dest }] }; }
        case 'UPDATE_CAB': { return { ...currentState, cabs: currentState.cabs.map(c => c.id === action.payload.id ? { ...c, ...action.payload, location: getCoords(action.payload.from, currentState), destination: getCoords(action.payload.to, currentState) } : c) }; }
        case 'DELETE_CAB': return { ...currentState, cabs: currentState.cabs.filter(c => c.id !== action.payload) };
        case 'ADD_DRIVER': return { ...currentState, drivers: [...currentState.drivers, { ...action.payload, id: Date.now(), role: 'driver' }] };
        case 'UPDATE_DRIVER': { return { ...currentState, drivers: currentState.drivers.map(d => d.id === action.payload.id ? { ...d, name: action.payload.name, phone: action.payload.phone, username: action.payload.username, password: action.payload.password || d.password } : d)}; }
        case 'DELETE_DRIVER': { const id = action.payload; return { ...currentState, drivers: currentState.drivers.filter(d => d.id !== id), cabs: currentState.cabs.map(c => c.driverId === id ? { ...c, driverId: null } : c) }; }
        case 'ADD_LOCATION': { const { name, lat, lon } = action.payload; if (currentState.locations.includes(name)) return currentState; const newCoords = { ...currentState.customLocationCoordinates, [name]: [parseFloat(lat), parseFloat(lon)] }; return { ...currentState, locations: [...currentState.locations, name].sort(), customLocationCoordinates: newCoords }; }
        case 'DELETE_LOCATION': { const loc = action.payload; const newPoints = { ...currentState.pickupPoints }; delete newPoints[loc]; const newCoords = { ...currentState.customLocationCoordinates }; delete newCoords[loc]; return { ...currentState, locations: currentState.locations.filter(l => l !== loc), pickupPoints: newPoints, customLocationCoordinates: newCoords }; }
        case 'ADD_POINT': { const { loc, point } = action.payload; return { ...currentState, pickupPoints: { ...currentState.pickupPoints, [loc]: [...(currentState.pickupPoints[loc] || []), point] } }; }
        case 'DELETE_POINT': { const { loc, point } = action.payload; return { ...currentState, pickupPoints: { ...currentState.pickupPoints, [loc]: currentState.pickupPoints[loc].filter(p => p !== point) } }; }
        case 'ADD_CUSTOMER': return { ...currentState, customers: [...currentState.customers, action.payload] };
        case 'ADD_TRIP': return { ...currentState, trips: [action.payload, ...currentState.trips] };
        case 'UPDATE_ADMIN_PASSWORD': { const { id, newPassword } = action.payload; return { ...currentState, admins: currentState.admins.map(a => a.id === id ? { ...a, password: newPassword } : a) }; }
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (action === 'update_state') {
            ...
            if (type === 'RESET_STATE') {
                state = JSON.parse(JSON.stringify(initialStateCopy));
            } else {
                state = appReducer(state, { type, payload });
            }
            return res.status(200).json(state);
        }
        ...
        if (action === 'login') {
            ...
            const dataSource = role === 'driver' ? state.drivers : state.admins;
        ...
        if (action === 'login-otp') {
            ...
            const user = state.admins.find(u => u.username === username);
        ...
        if (action === 'change-password') {
            ...
            // The password check is now done on the client, so we just apply the change.
            // In a real app, this would re-verify the current password.
            const admin = state.admins.find(a => a.id === userId);
